...
public class MaximumSubarraySumTest {
    ...
    public void testNegativeExampleArray() throws Exception {
        assertEquals("Example array with all negative values should have a max of 0", 0, MaximumSubarraySum.sequence(new int[]{-2, -1, -3, -4, -1, -2, -1, -5, -4}));
    }

    @Test
    public void testCompleteArray() throws Exception {
        assertEquals("Should work on this too", 155, MaximumSubarraySum.sequence(new int[]{7, 4, 11, -11, 39, 36, 10, -6, 37, -10, -32, 44, -26, -34, 43, 43}));
    }
    ...
    public void testRandomArrays() throws Exception {
        for (int i = 0; i < 50; i++) {
            ...
            assertEquals("Random array solution not as expected: ", solution(rand), MaximumSubarraySum.sequence(rand));
        }
    }
